...
> 由 Cowork 對 `index.html`（4438 行）程式碼掃描產出，供 Claude Code 接手。
...
| ~~B5~~ | ✅ **已完成（P0）** 35 處 `console.*` 收斂到 debug flag（`?debug=1`） | 整潔 | ⚪ | S | — | index.html:459 單點攔截，預設靜默、可開關 |
...
| B11 | 4438 行單檔內函式分組可導覽（章節 anchor/索引） | 可維護 | ⚪ | S | CW | 已由 AGENTS.md Code Map 部分覆蓋 |
...
- BOOT（行 ~4426）：`loadFromStorage().then(render)`，維持 cloud-first 覆蓋順序。

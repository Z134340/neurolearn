...
使用者操作（書籤 / 完成測驗 / 上傳題庫 ...）
         ...
         ▼
   saveToStorage()
         ├── localStorage（key: neurolearn_v2_{user_uid}）  ← 完整 blob（單機真實來源）
         │     QuotaExceeded 時三段降級：完整 → 移除 explanation → 僅 metadata
         │
         └── scheduleSyncToCloud()  ← 60 秒（輕量）/ 5 分鐘（全量）debounce
                    │                  quota 超限自動暫停 30 分鐘（_syncPausedUntil）
                    ▼
              Firebase SDK（分層寫入，非單一文件）
                    │
                    ▼
           Firestore (Cloud)  ─ Firebase Auth + Security Rules
        ┌──────────────────────────────────────────────┐
        │ users/{uid}                （主狀態文件）       │
        │   bookmarks, wrong_qs, used_q_ids,             │
        │   dash_tag, tag_filter, current_mat,           │
        │   read_progress, updated_at                    │
        │                                                │
        │ users/{uid}/exam_files/{fid}      （每檔一文件）│
        │   id, name, tags, uploaded_at                  │
        │                                                │
        │ users/{uid}/exam_questions/{q_id} （每題一文件）│
        │   q_id, file_id, exam, question,               │
        │   option_a~f, answer, explanation              │
        │                                                │
        │ users/{uid}/quiz_sessions/{id}    （每次測驗）  │
        │   history（wrongIds / bkIds）, created_at       │
        │                                                │
        │ users/{uid}/study_materials/{id}  （每篇教材）  │
        │   id, title, raw, sections, toc, mat_tags,     │
        │   uploaded_at                                  │
        │                                                │
        │ users/{uid}/study_annotations/{id}（每筆筆記）  │
        │   id, mat_id, sec_id, tag, memo, html,         │
        │   created_at                                   │
        └──────────────────────────────────────────────┘
                    │
                    ▼ onSnapshot（逐 subcollection 即時監聽）
         其他裝置自動收到更新

> 註：六個 subcollection 皆雙向同步（`syncStudyToCloud` / `loadStudyFromCloud`
> 負責 study_materials 與 study_annotations）。localStorage 為單機完整快取／離線
> 後援，Firestore 為跨裝置正規化真實來源。
```
...
```bash
cd ~/NeuroLearn && bash scripts/setup_proxy.sh
```
...
| 檔案 | 說明 |
|------|------|
| `scripts/claude_proxy.py` | Proxy 主程式，監聽 port 7734 |
| `scripts/com.neurolearn.proxy.plist` | macOS launchd 參考模板（實際由 setup 動態產生）|
| `scripts/setup_proxy.sh` | 一鍵安裝腳本（動態產生 plist，無硬編碼路徑）|
...
儲存分為兩層：**localStorage 為單機完整真實來源**，**Firestore 為跨裝置正規化同步層**。兩者欄位命名與結構不同，請勿混為一談。

#### Layer 1 — localStorage（單一 blob，key：`neurolearn_v2_{uid}`）

`saveToStorage()` 將下列全部欄位序列化為單一 JSON 寫入：

| 欄位 | 內容 | 上雲 |
|------|------|:----:|
| `history` | 測驗歷史（含 wrongIds / bkIds）| ✅ quiz_sessions |
| `bookmarks` | 已標記題目 ID | ✅ users doc |
| `wrongQs` | 累計錯誤題目 ID | ✅ users doc |
| `usedQIds` | 已出過的題目 ID（不重複出題）| ✅ users doc |
| `files` | 資料集元資料（不含二進制）| ✅ exam_files |
| `extraQs` | 完整題庫題目（解析後）| ✅ exam_questions |
| `dashTag` / `tagFilter` | 儀表板 / 資料集篩選標籤 | ✅ users doc |
| `currentMat` / `readProgress` | 教材閱讀位置 | ✅ users doc |
| `materials` | 教材內容（完整 Markdown）| ✅ study_materials |
| `annotations` | 段落筆記標記 | ✅ study_annotations |

> localStorage 上限約 5 MB；`materials` / `extraQs` 為最大宗。超限時三段降級（完整 → 移除 explanation → 僅 metadata），降級後題目／教材內容改由 Firestore 提供。

#### Layer 2 — Firestore（subcollection 正規化）

| 路徑 | 粒度 | 主要欄位 |
|------|------|---------|
| `users/{uid}` | 帳號單一文件 | bookmarks, wrong_qs, used_q_ids, dash_tag, tag_filter, current_mat, read_progress, updated_at |
| `users/{uid}/exam_files/{fid}` | 每檔一文件 | id, name, tags, uploaded_at |
| `users/{uid}/exam_questions/{q_id}` | 每題一文件 | q_id, file_id, exam, question, option_a~f, answer, explanation |
| `users/{uid}/quiz_sessions/{id}` | 每次測驗一文件 | history（wrongIds / bkIds）, created_at |
| `users/{uid}/study_materials/{id}` | 每篇教材一文件 | id, title, raw, sections, toc, mat_tags, uploaded_at |
| `users/{uid}/study_annotations/{id}` | 每筆筆記一文件 | id, mat_id, sec_id, tag, memo, html, created_at |

...
│   ├── 標籤管理（新增 / 移除 / 篩選）
│   ├── 題目出處記錄（來源檔名 / 題號）
│   └── 下載「題目擴充範本.xlsx」（SheetJS 產生，含欄寬設定）
│
├── 🎯 模擬考
│   ├── ① 選擇題目分類（多選 / 全選 / 清除）
│   │
...
在「資料集」頁面點擊「題目範本」按鈕，即可下載 **`題目擴充範本.xlsx`**（由 SheetJS 在瀏覽器端即時產生，含欄寬格式設定）。
...
| **框架** | 無（Vanilla JS）|
| **外部依賴** | Supabase JS SDK（`defer`）、Marked.js、SheetJS（xlsx.full.min.js）|
...
### v2.1
- **[修正]** 模擬考結果頁逐題明細：多選題答對時仍顯示 ❌ 的問題（`===` 陣列參照比較改為 `isCorrectAnswer()`）
- **[新增]** 引入 SheetJS（`xlsx.full.min.js`），XLSX 上傳解析正式生效
- **[新增]** 題目範本改為下載 `題目擴充範本.xlsx`（含欄寬格式，取代原 CSV 範本）
